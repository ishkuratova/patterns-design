...
class EmployeeRep(ABC):
    ...
    def get_k_n_short_list(self, k: int, n: int) -> List['Employee']:
        ...
        if start_index >= len(self._employees):
            raise IndexError("Страница за пределами диапозона")
    ...
    def sort_by_field(
            self, field: str, reverse: bool = False
    ) -> List['Employee']:
        ...
        if field not in valid_fields:
            raise ValueError(
                ...
                f"Допустимые поля: {valid_fields}"
            )
    ...
    def add_employee(self, first_name: str, last_name: str, salary: int,
                     passport: str, patronymic: str | None = None
                     ) -> Employee:
        ...
        new_employee = Employee(
            new_id, first_name, last_name, patronymic, salary, passport
        )

        # Проверка уникальности паспорта
        for existing_employee in self._employees:
            if existing_employee == new_employee:
                raise ValueError(
                    f"Сотрудник с номером паспорта '{passport}' уже существует"
                )
    ...
    def update_employee(self, employee_id: int, **kwargs) -> bool:
        ...
        valid_fields = [
            'first_name', 'last_name', 'patronymic', 'salary', 'passport'
        ]
        for field, value in kwargs.items():
            if field in valid_fields:
                if field == 'salary':
                    ...
                    # Проверка уникальности паспорта при обновлении
                    if value != employee.passport:
                        temp_employee = Employee(0, "", "", "", 0, value)
                        for existing_employee in self._employees:
                            if (existing_employee != employee and
                                    existing_employee == temp_employee):
                                raise ValueError(
                                    f"Сотрудник с номером паспорта "
                                    f"'{value}' уже существует"
                                )
...
class DatabaseConnection:
    ...
    def __new__(cls, host: str, port: int, database: str, user: str,
                password: str):
...
class EmployeeRepDB:
    ...
    def get_by_id(self, employee_id: int) -> Employee | None:
        ...
        query = """
        SELECT employee_id, first_name, last_name, patronymic, salary, passport
        FROM employees
        WHERE employee_id = %s
        """
    ...
    def update_employee(self, employee_id: int, **kwargs) -> bool:
        ...
        try:
            ...
            if "unique constraint" in str(error).lower():
                raise ValueError(
                    "Сотрудник с таким номером паспорта уже существует"
                ) from error
...
class EmployeeRepDBAdapter(EmployeeRep):
    ...
    def add_employee(self, first_name: str, last_name: str, salary: int,
        ...
        new_employee = self._db.add_employee(
...
class EmployeeRepDecorator:
    ...
    def get_k_n_short_list(self, k: int, n: int, filter_func: callable = None,
                           sort_field: str = None, reverse: bool = False
                           ) -> List['Employee']:
        """Возвращает отфильтрованную и отсортированную страницу."""
        employees_list = self._file_repo.get_k_n_short_list(
            self._file_repo.get_count(), 1
        )

        if filter_func:
            employees_list = [
                emp for emp in employees_list if filter_func(emp)
            ]
        ...
        if sort_field:
            ...
            if sort_field not in valid_fields:
                raise ValueError(
                    f"Недопустимое поле для сортировки: {sort_field}"
                )

            if sort_field == 'employee_id':
                employees_list.sort(
                    key=lambda x: x.employee_id, reverse=reverse
                )
            elif sort_field == 'first_name':
                employees_list.sort(
                    key=lambda x: x.first_name, reverse=reverse
                )
            elif sort_field == 'last_name':
                employees_list.sort(
                    key=lambda x: x.last_name, reverse=reverse
                )
            elif sort_field == 'salary':
                employees_list.sort(
                    key=lambda x: x.salary, reverse=reverse
                )
